...
func (p *javascriptProvider) BlastRadius(ctx context.Context, src WorkflowSource, path string, line, char int) (map[string]any, error) {
	...
	if raw, ok := src.References(ctx, path, line, char); ok {
		if locs, err := p.ParseLocations(raw); err == nil {
			...
			jsSortRefs(path, test)
			jsSortRefs(path, nontest)
...
func (p *javascriptProvider) SimulateEdit(ctx context.Context, src WorkflowSource, path string, edits []TextEdit) (map[string]any, error) {
	...
	restored = true
	introduced, resolved := jsDiagDiff(before, after)
...
// jsSortRefs 引用排序: 同目录优先 → 路径浅优先 → 服务器原始顺序保持
// (截断时保重要引用, 非返回顺序)。
func jsSortRefs(queryPath string, in []map[string]any) {
...
// jsDiagDiff 诊断差集 (before→after): introduced=新增消息, resolved=消除消息 (按 message 多重集差分)。
func jsDiagDiff(before, after map[string]any) (introduced, resolved []string) {
